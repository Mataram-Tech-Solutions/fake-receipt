...
class JsnController extends GetxController {
  //TODO: Implement JsnController
  var asal1 = ''.obs;
  var tanggal = ''.obs;
  var waktu = ''.obs;
  var kode = ''.obs;
  ...
  Future<void> saveData() async {
    ...
    try {
      if (connectionStatus) {
        ...
        final ByteData data = await rootBundle.load('assets/jsn_print.png');
        final Uint8List bytes = data.buffer.asUint8List();
     ...
     final Uint8List resizedBytes = img.encodePng(resizedImage);
        ...
        bytesToPrint += generator.text(
          'Info Tol (0271) 6882222 ', // Teks normal
          styles: PosStyles(
            align: PosAlign.center,
            bold: false,
            height: PosTextSize.size1, // Ukuran paling kecil
        ...
        // Panjang ideal untuk ASAL GERBANG (misal: 20 karakter)
        const int idealLengthgr = 20;

        // Dapatkan panjang dari asal2.value
        int asalLengthgr = asal2.value.length;

        // Hitung jumlah spasi yang dibutuhkan
        // Jika panjang asal2 lebih dari idealLength, spasi berkurang.
        // Jika panjang asal2 kurang dari idealLength, spasi bertambah.
        int extraSpacesgr = (idealLengthgr - asalLengthgr).clamp(0, 10); // Minimal 0, maksimal 10 spasi

        // Buat string spasi sesuai perhitungan
        String spacesgr = ' ' * extraSpacesgr; // Membuat spasi sebanyak extraSpaces

        // Tambahkan teks dan spasi
        bytesToPrint += generator.text(
          'ASAL GERBANG: ${asal2.value}$spacesgr', // Teks dengan spasi tambahan
          styles: PosStyles(
            align: PosAlign.center,
            bold: false,
            height: PosTextSize.size1, // Ukuran paling kecil
